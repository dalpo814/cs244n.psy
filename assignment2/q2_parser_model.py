...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        self.pretrained_embeddings # 50-dimensional embedding
        tf.nn.embedding_lookup(embedding_matrix, ids) = embedding_matrix[ids]        
        병렬처리가 들어가서 속도가 빠름
        
        input_placeholder 예시 = [5156 99 207 224] 1 x n_features
        tf.nn.embedding_lookup(embedding_matrix, input_placeholder) = [[w_5156], [w_99], [w_207], [w_224]] 1 x n_features x embedding_size
        
        Hint: You can use tf.reshape to concatenate the vectors. See following link to understand
            what -1 in a shape means.
            https://www.tensorflow.org/api_docs/python/array_ops/shapes_and_shaping#reshape.
            tf.reshape(tensor, shape)으로 사용하며, 
            tf.reshape(tensor, [-1]) 은 flatten 시키는 것
            tf.reshape(tensor, [2, -1])는 전체 shape을 기반으로 추정하여 자동으로 대입시켜 줌
