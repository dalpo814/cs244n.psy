...
def softmax(x):
    ...
    sum = tf.reduce_sum(tf.exp(x - max), axis=1, keep_dims=True)
    out = tf.exp(x - max) / sum
    ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    y = tf.to_float(y)
    out = -tf.reduce_sum(tf.multiply(y, tf.log(yhat))) # multiply : element-wise multiplication
