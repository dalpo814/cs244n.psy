    ...
    def parse_step(self, transition):
        ...
        if transition == "S":  # buffer에서 제일 첫번째의 단어를 빼서 stack으로 넘김
            ...
            if(len(self.stack) >= 2):
                ...
                self.stack = tmp # dependent를 stack에서 제외(-2번째)              
        elif transition == "RA":
            if(len(self.stack) >= 2):
                self.dependencies.append((self.stack[-2], self.stack[-1])) # (head, dependent)쌍을 dependent배열에 저장
                self.stack = self.stack[:-1] # dependent를 stack에서 제외(-1번째)
        else:
            raise ValueError
...
def minibatch_parse(sentences, model, batch_size):    
    ...
    어차피 모델은 predict를 한 번에 하나의 transition을 내놓을 수 밖에 없는데,
    여러 개의 문장에 대해 병렬 처리를 하기 원하기 때문에 이 function이 개발됨
    ...
    num_iter = int(len(sentences) / batch_size)    
